...
1. 升级到 5.x.x,由于底层数据库升级,所有本地数据将会失效,有三种方法恢复数据:
   1. 开启私有数据云备份,程序会自动进行数据的同步。
   1. 版本升级前，使用数据库导出,版本升级后，选择导出的数据库文件进行数据库恢复。
   1. 版本升级前，使用全量导出功能导出数据,版本升级后，将导出的数据重新导入。

### ⭐ Added

- 优化Pglite数据库操作的正确性(避免并发调用时的事务交错或顺序混乱)

### 💣 BREAKING CHANGES

- 升级 pglite 至 0.5.x (PG 17 to PG 18)

## 4.2.2 (2026-06-12)

### 🐛 Fixed

- 修复BOSS直聘列表第15个招聘信息之后的部分没有触发插件的问题
...
1. 升级到 4.x.x,由于[底层数据库升级](https://pglite.dev/docs/upgrade#upgrading-pglite-from-v0-2-x-to-v0-3-x),所有本地数据将会失效,有两种方法恢复数据:
   1. 开启数据共享计划,重新添加伙伴后,程序会自动进行数据的同步。
   2. 版本升级前，使用全量导出功能（职位数据，公司数据，职位标签数据，公司标签数据，职位快照，职位公开数据，公司评论）导出数据,版本升级后，将导出的数据重新导入.
...
- 新增数据源模块
- 新增数据源元数据（内置数据源列表含有*公司评论数据*，需自行选择）
- 新增公开数据（职位）共享与合并到私有的职位数据（当前增强职位初见时间）
- 新增后台页面框架面包屑显示当前路由页面标题
- 职位卡片新增公司评论检测

### 🩹 Fixes

- 拆分'数据共享计划'为'数据源'和'任务'
- 修改首页布局，新增常用功能导航，高级功能导航
- 补全菜单图标
- 原*公司评论*改为*在线公司评论*
- git 仓库文件下载由 GitHub Api 改为 git https api

### ❤️ Thank You

- lastsunday @lastsunday
- lizhichen-chen @lizhichen-chen

## 3.12.2 (2025-04-28)

### 🩹 Fixes

- 修复职位页滚动异常的问题（BOSS 直聘）
...
- 隐藏职位卡片空数据的公司标签，职位标签
- 修复职位卡片的公司官网的可达性逻辑
- 适配 BOSS 直聘关键字查询（根据公司名）职位卡片渲染逻辑
- 减少查询公司信息的手动点击次数
...
- 修复 BOSS 直聘改版导致插件渲染失败的问题（搜索页/推荐页）
...
- 移除新版(20250416)BOSS 直聘推荐/搜索页面的职位排序（避免一次性触发加载多页数据）
- 适配新版(20250416)BOSS 直聘推荐/搜索页面
...
- 为了避免违反相关法律法规，请关闭数据共享计划，并将相关的 job-hunting-data 仓库关闭（或删除）
...
- 新增 e2e tests(base on playwright-bdd)
- 提升程序启动的稳定性(对 Bridge 进行重试)
...
- 调整插件安装文件的内部目录结构，使其可直接安装（通过直接拖动 zip 文件到浏览器扩展页面的方式进行安装）
